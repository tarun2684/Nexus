...
class ComboManager:
    ...
    def calculate_rapid_combo(
        self, completion_times: list[datetime], reference_time: datetime | None = None
    ) -> ComboInfo:
        """
        ...
        max_count = self._find_max_combo_in_window(completion_times, reference_time)
    ...
    def _find_max_combo_in_window(
        self, completion_times: list[datetime], reference_time: datetime
    ) -> int:
    ...
    def can_start_category_combo(
        self, category: str, recent_completions: list[tuple[str, datetime]], required_count: int = 3
    ) -> bool:
        """
        ...
        category_count = sum(
            1 for cat, time in recent_completions if cat == category and time >= window_start
        )
