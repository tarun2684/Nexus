...
class GameService:
    ...
    def complete_quest(
        ...
        effective_coins = (
            int(quest_coins / penalty_multiplier) if penalty_multiplier > 1.0 else quest_coins
        )
        ...
        if combo_info.is_active:
            message_parts.append(
                f"⚡ Combo: {combo_info.current_count}x ({combo_info.multiplier}x multiplier)"
            )
    ...
    def calculate_quest_rewards(
        ...
        return {
            ...
            "combo_multiplier": (
                self.combo_manager._get_rapid_multiplier(combo_count) if combo_count >= 2 else 1.0
            ),
